...
/*
  THE FRAME, stated once because getting it wrong mirrors the whole world.

    core = ( ECI_X , -ECI_Z , -ECI_Y )

  p5's WEBGL space is LEFT-handed: +x right, +y DOWN, +z toward the viewer,
  so x cross y points away from the viewer, not toward it. The core frame is
  therefore deliberately left-handed too (det of ECEF -> core is -1). An
  earlier version was right-handed, which rendered every map mirrored in
  both builds while staying self-consistent, so orbits and coastlines agreed
  with each other and only the geography was wrong.

  -ECI_Z puts celestial north at -y, which is up on a y-down screen.
  Three.js is right-handed and y-up, so it converts as
    three = ( core_x , -core_y , core_z )
  which composes to det +1 there. Both are checked in the suite.
*/
