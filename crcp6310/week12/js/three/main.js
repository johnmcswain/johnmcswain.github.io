...
import { sunEphemeris, inShadow } from '../sim/sun.js';
import { pickNotables } from '../sim/notables.js';
import { findClosePairs, ConjunctionWatch } from '../sim/conjunctions.js';
import { moonState, eclipticToP5, moonDrawDistanceKm,
         MOON_RADIUS_KM } from '../sim/planets.js';
...
import { MagnetosphereThree } from './solids.js';
...
class OrbitaThree {
  constructor(parent) {
    ...
    /* log-compressed size (see sim/solarwind.js): the true 109x ratio would
       swallow the scene at this compressed distance, and true angular size
       would make it smaller than Earth */
    this.sun = new Sun(sunDrawnRadiusKm() * KM2U);
    ...
    this.moon = new THREE.Mesh(
      new THREE.SphereGeometry(Math.max(2, MOON_RADIUS_KM * KM2U), 28, 20),
    ...
    /* earth-fixed, so it rides the rotating group with the coastlines */
    this.magneto = new MagnetosphereThree(KM2U);
    this.earth.group.add(this.magneto.group);

    this.wind = new WindPoints(260, KM2U);
    this.scene.add(this.wind.points, this.wind.ring, this.wind.tail);

    this.sonifier = new Sonifier();
    this.tracker = new SkyTracker();
    this.conjWatch = new ConjunctionWatch();
    this.weather = { ...QUIET_BASELINE, liveFields: 0, totalFields: 6 };
    this.windSeries = null; this.windSampleT = 0;
    this.xraySeries = null; this.flareGlow = 0;
    this.lastFlare = null; this.currentClass = null; this.lastSimT = 0;
    this.substorm = new SubstormCycle();
    this.flareWatch = new FlareWatch();
    ...
    this.idleMs = 0;
    /* the far-field layers live 15+ Earth radii out; toggling one eases the
       camera out to frame it, then releases control back to OrbitControls */
    this.dollyTarget = 0; this.dollyFrames = 0;
  ...
  async loadWeather() {
    ...
    try { this.windSeries = await swpc.loadSeries(); } catch { this.windSeries = null; }
    try { this.xraySeries = await swpc.loadXray(readXraySeries); } catch { this.xraySeries = null; }
    this.hud();
    try {
  ...
  onKey(e) {
    ...
    else if (k === 'r') { this.wind.visible = !this.wind.visible; this.frameFar(); this.hud(); }
    else if (k === 'b') { this.magneto.visible = !this.magneto.visible; this.frameFar(); this.hud(); }
    else if (k === 'd') { this.demo.toggle(); this.hud(); }
  ...
  /* aim the camera at whichever scale is now showing */
  frameFar() {
    const far = this.wind.visible || this.magneto.visible;
    const spanUnits = far ? 16 * R_EARTH_KM * KM2U
                          : scaledRadiusKm(CONFIG.altMaxKm, altExag()) * KM2U;
    this.dollyTarget = spanUnits / Math.tan(this.camera.fov * Math.PI / 360) * 1.05;
    this.dollyFrames = 70;
  }
  ...
  frame() {
    ...
    this.sun.setDirection(sun.eciDir[0], -sun.eciDir[1], sun.eciDir[2],
      SUN_DISTANCE_RE * R_EARTH_KM * KM2U);       // beyond the wind inflow
    this.earth.setGMST(sun.gmstDeg * Math.PI / 180);
    this.earth.setSunDirection(sun.eciDir[0], -sun.eciDir[1], sun.eciDir[2],
                               sun.gmstDeg * Math.PI / 180);
    this.aurora.update(this.auroraKp ?? this.weather.kp, sun.subsolarLonDeg,
      { gain: this.substorm.auroraGain, polewardDeg: this.substorm.polewardDeg });
    this.magneto.update(dtReal / 1000);
    ...
    const mR = moonDrawDistanceKm(mo.distKm) * KM2U;
    ...
    /* replay the propagated hour so the boundary breathes on measured data */
    const dSim = Math.max(0, Math.min(3600, (state.simT - this.lastSimT) / 1000));
    this.lastSimT = state.simT;

    /* DEMONSTRATION MODE (D): invented conditions, labelled everywhere they
       appear. The measured values are never overwritten. */
    const demoC = state.paused ? null : this.demo.update(dSim);
    const liveWind = sampleSeries(this.windSeries, state.simT);
    const conditions = this.demo.active && demoC ? { ...this.weather, ...demoC }
                     : liveWind ? { ...this.weather, ...liveWind } : this.weather;
    this.auroraKp = this.demo.active && demoC ? demoC.kp : this.weather.kp;
    this.windSampleT = liveWind && !this.demo.active ? liveWind.tMs : 0;
    if (!state.paused && this.substorm.update(dSim, conditions) === 'onset')
      this.sonifier.crossing(220, true);

    /* flares replayed from the GOES series */
    const xr = this.demo.active && demoC ? { flux: demoC.flux }
      : sampleSeries(this.xraySeries, state.simT);
    if (xr) {
      this.currentClass = flareClass(xr.flux);
      const ev = state.paused ? null : this.flareWatch.update(xr.flux);
      if (ev) { this.lastFlare = ev; this.flareGlow = 1; this.sonifier.ping(1400, 1900); }
    }
    this.flareGlow = Math.max(0, this.flareGlow - dtReal / 2600);
    const flare = this.flareGlow * (this.lastFlare ? flareIntensity(this.lastFlare.flux) : 0);
    /* bloom carries the flash: pushing the emissive above 1 is what makes
       UnrealBloom bloom rather than merely brighten */
    this.sun.mesh.material.color.setRGB(1 + 2.2 * flare, 0.93 + 1.3 * flare, 0.76);
    this.sun.mesh.scale.setScalar(1 + 0.3 * flare);
    this.sun.light.intensity = 2.6 * (1 + 1.6 * flare);

    this.wind.update(dtReal / 1000, conditions, sun.eciDir, this.substorm.tailStretch);
    ...
    if (!state.paused && objs.length > 1) {
      const pairs = this.conjWatch.update(
        findClosePairs(this.headsKm, objs.length, CONFIG.conjKm));
    ...
    if (this.dollyFrames > 0) {
      this.dollyFrames--;
      const d = this.camera.position.length();
      const k = d + (this.dollyTarget - d) * 0.08;
      this.camera.position.multiplyScalar(k / (d || 1));
    }
  ...
  hud() {
    ...
    setText('s-conj', this.conjCount + ' encounters \u00b7 '
      + this.conjWatch.activeCount + ' close now');
    setText('s-fps', this.fps ? this.fps + ' fps' : '\u2014');
    const w = this.weather;
    const dbanner = $('demobanner');
    if (dbanner) {
      dbanner.style.display = this.demo.active ? 'block' : 'none';
      if (this.demo.active) dbanner.textContent = DEMO_LABEL
        + ' \u00b7 storm scenario, ' + Math.round(this.demo.minutes % SCENARIO_MINUTES)
        + '/' + SCENARIO_MINUTES + ' min';
    }
    setText('s-kp', this.demo.active ? 'SIMULATED \u2014 demonstration conditions'
      : 'Kp ' + w.kp.toFixed(1) + ' \u00b7 G' + w.scaleG + ' \u00b7 '
    ...
    const wl = $('wxlegend');
    if (wl) wl.style.display = this.wind.visible ? 'block' : 'none';
    setText('s-wind', this.wind.visible
      ? 'magnetopause ' + this.wind.standoffRe.toFixed(1) + ' R\u2091 \u00b7 flaring '
        + this.wind.alpha.toFixed(2) + ' \u00b7 ' + this.wind.activeCount
        + ' parcels (flow scaled)' : 'hidden');
    setText('s-storm', this.substorm.phase + ' \u00b7 load '
      + Math.round(this.substorm.loadFraction * 100) + '% \u00b7 gain '
      + this.substorm.auroraGain.toFixed(2) + ' \u00b7 '
      + this.substorm.onsets + ' onsets');
    setText('s-xray', this.currentClass
      ? this.currentClass.label
        + (this.lastFlare ? ' \u00b7 last flare ' + this.lastFlare.label : '')
        + ' \u00b7 ' + this.flareWatch.count + ' this session'
      : (this.xraySeries ? 'waiting' : 'no X-ray series'));
    setText('s-swseries', this.windSeries
      ? (swpc.seriesSource || 'series') + ' \u00b7 ' + this.windSeries.length + ' samples'
        + (this.windSampleT ? ' \u00b7 replaying ' + new Date(this.windSampleT)
            .toISOString().slice(11, 16) + 'Z' : '')
      : 'snapshot only \u2014 ' + (swpc.seriesError || 'not loaded'));
    setText('s-frame', (this.wind.visible || this.magneto.visible)
      ? 'wide \u2014 fits 24 R\u2091 (magnetotail)' : 'close \u2014 orbital shells');
    setText('s-belt', this.magneto.visible
      ? this.magneto.parts + ' objects (toroid, helix, shell)' : 'hidden');
