...
(() => {
  ...
  function nodalRegression(aKm, e, inclDeg) {
    ...
    const p = aKm * (1 - e * e);
    const rate = -1.5 * n * J2 * (R_EARTH_KM3 / p) ** 2 * Math.cos(inclDeg * D2R2);
    return rate * 86400 / D2R2;
  }
  var F107_QUIET = 70;
  ...
  function dragEmphasis(bstar, altKm, f107 = F107_QUIET) {
  ...
  function scaledRadiusKm(altKm, exag = 1) {
    return R_EARTH_KM4 + Math.max(0, altKm) * exag;
  }
  function orbital3D(inclDeg, raanDeg, uDeg, r, out, o = 0) {
    const i = inclDeg * D2R3, O = raanDeg * D2R3, u = uDeg * D2R3;
  ...
  var SEGS = 3;
  var EMPHASIS_LEVELS = 3;
  var emphasisPools = Array.from({ length: EMPHASIS_LEVELS }, () => new Pool());
  var EMPHASIS_STYLE = [[9, 26], [13, 40], [18, 58]];
  ...
  var orbitsMotif = {
    ...
    build(ctx) {
      ...
      for (let idx = 0; idx < objects.length; idx++) {
        ...
        const emphLevel = emph > 0.34 ? Math.min(EMPHASIS_LEVELS - 1, Math.floor((emph - 0.34) / 0.22)) : -1;
  ...
  function latLonToXYZ(latDeg, lonDeg) {
    const la = latDeg * D2R4, lo = lonDeg * D2R4;
  ...
  function drawEarth(p, rPx, sunEF) {
    ...
    p.noStroke();
    if (sunEF) {
      p.ambientLight(30, 36, 52);
      p.directionalLight(168, 152, 124, -sunEF[0], -sunEF[1], -sunEF[2]);
      p.ambientMaterial(78, 96, 126);
    } else {
      p.fill(14, 18, 27);
    }
  ...
  function readSeries(payload, fields) {
    ...
    if (Array.isArray(payload) && Array.isArray(payload[0])) {
      const header = payload[0].map((h) => String(h).toLowerCase().replace(/[_\s]/g, ""));
      const idx = {};
      ...
      const find = (part) => keys.find((k) => k.toLowerCase().replace(/[_\s]/g, "").includes(part));
  ...
  var SpaceWeatherFeed = class {
    ...
    async #tryJson(url) {
      ...
      try {
        ...
        this.#failed.add(url);
        return { err: "blocked or unreachable" };
      }
    }
    ...
    async loadSeries() {
      ...
      const wind2 = await this.#tryJson(`${BASE}/json/rtsw/rtsw_wind_1m.json`);
      if (!wind2.json) {
        this.seriesError += `; rtsw ${wind2.err}`;
        return null;
      }
      const wSeries = readSeries(wind2.json, { speedKmS: "speed", density: "density" });
      if (!wSeries) {
        this.seriesError += "; rtsw shape not recognised";
        return null;
      }
      ...
      if (bSeries) {
        ...
        for (const row of wSeries) {
          while (j < bSeries.length - 1 && Math.abs(bSeries[j + 1].tMs - row.tMs) < Math.abs(bSeries[j].tMs - row.tMs)) j++;
  ...
  function ovalIntensity(kp, latDeg, lonDeg, subsolarLonDeg, opts = {}) {
    ...
    const base = Math.max(0, Math.min(1, band * night2 * (0.25 + kp / 9 * 0.75)));
  ...
  var BUCKETS = [
    [0.06, 70, 235, 150, 170, 3.2],
    [0.26, 120, 255, 175, 225, 4.2],
    [0.5, 205, 255, 200, 255, 5.2],
    [0.74, 255, 165, 150, 255, 6],
    [1.1, 255, 232, 210, 255, 8]
    // substorm breakup: brighter than quiet
  ];
    ...
    update(kp, subsolarLonDeg, storm = {}) {
      ...
      if (this.#ovation) {
        ...
        for (let i = 0; i < pts.length; i += 3)
          this.#place(pts[i], pts[i + 1], pts[i + 2] * g);
        return;
      }
  ...
  // src/sim/solarwind.js
  var R_EARTH_KM6 = 6371;
  var START_RE = 15;
  var SUN_DISTANCE_RE = 19.5;
  var R_SUN_KM = 695700;
  var SUN_LOG_GAIN = 1;
  function sunDrawnRadiusKm(earthRadiusKm = 6371) {
    return earthRadiusKm * (1 + SUN_LOG_GAIN * Math.log10(R_SUN_KM / earthRadiusKm));
  }
  var REF_TRANSIT_S = 9;
  var REF_SPEED = 400;
  function magnetopauseStandoffRe(density, speedKmS) {
    const n = Math.max(0.1, density), v2 = Math.max(50, speedKmS);
    return 107.4 * Math.pow(n * v2 * v2, -1 / 6);
  }
  function dynamicPressureNPa(density, speedKmS) {
    return 16726e-10 * Math.max(0.01, density) * Math.max(1, speedKmS) ** 2;
  }
  function flaringAlpha(bzNt, density, speedKmS) {
    const dp = dynamicPressureNPa(density, speedKmS);
    return (0.58 - 7e-3 * bzNt) * (1 + 0.024 * Math.log(Math.max(0.05, dp)));
  }
  function magnetopauseRadiusRe(standoffRe, thetaRad, alpha) {
    const c = Math.cos(Math.min(Math.PI * 0.92, Math.max(0, thetaRad)));
    return standoffRe * Math.pow(2 / (1 + c), alpha);
  }
  function visualTransitSec(speedKmS) {
    return REF_TRANSIT_S * REF_SPEED / Math.max(50, speedKmS);
  }
  function bzColour(bzNt, out) {
    const t = Math.max(0, Math.min(1, -bzNt / 12));
    out[0] = 0.34 + 0.62 * t;
    out[1] = 0.62 - 0.34 * t;
    out[2] = 0.96 - 0.52 * t;
    return out;
  }
  var RING_THETA = [22, 46, 70, 94, 116];
  var RING_AZ = 40;
  var MERID = 6;
  var MERID_STEPS = 14;
  var MERID_MAX = 124;
  function magnetopauseWireframe(standoffRe, alpha, sunDir, basis, out) {
    const [ux, uy, uz, vx, vy, vz] = basis;
    let n = 0;
    const put = (theta, phi, o) => {
      const r = magnetopauseRadiusRe(standoffRe, theta, alpha) * R_EARTH_KM6;
  ...
  var MAGNETOPAUSE_SEGMENTS = RING_THETA.length * RING_AZ + MERID * MERID_STEPS;
  var TAIL_MAX_RE = 22;
  var TAIL_RADIUS_RE = 21;
  var TAIL_RINGS = 7;
  var TAIL_AZ = 32;
  var TAIL_LINES = 8;
  var TAIL_SEGMENTS = TAIL_RINGS * TAIL_AZ + TAIL_LINES * TAIL_RINGS;
  function tailRadiusRe(flankRe, xRe, stretch = 0) {
    const t = Math.max(0, Math.min(1, xRe / TAIL_MAX_RE));
    const target = TAIL_RADIUS_RE * (1 - 0.14 * stretch);
    return flankRe + (target - flankRe) * Math.pow(t, 0.55);
  }
  function tailWireframe(standoffRe, alpha, stretch, sunDir, basis, out) {
    const flank = magnetopauseRadiusRe(standoffRe, Math.PI / 2, alpha);
    const len = TAIL_MAX_RE * (0.72 + 0.28 * stretch);
    const [ux, uy, uz, vx, vy, vz] = basis;
    let n = 0;
    const put = (xRe, phi, o) => {
      const r = tailRadiusRe(flank, xRe, stretch) * R_EARTH_KM6;
      const ax = -xRe * R_EARTH_KM6;
  ...
  function sampleSeries(series, tMs) {
  ...
  var WindStream = class {
    ...
    #respawnLateral(i) {
      this.#lr[i] = Math.sqrt(this.#rand()) * 3.4;
    ...
    step(dtSec, { speedKmS = 400, density = 5 } = {}, sunDir, out) {
      ...
      basisFromNormal2(sunDir[0], sunDir[1], sunDir[2], this.#basis);
      const [ux, uy, uz, vx, vy, vz] = this.#basis;
      for (let i = 0; i < n; i++) {
        let u = this.#u[i] + du;
        if (u >= 1) {
        ...
        this.#u[i] = u;
        const dist = (START_RE - (START_RE - this.standoffRe) * u) * R_EARTH_KM6;
        const lat = this.#lr[i] * (1 + 1.9 * u * u) * R_EARTH_KM6;
  ...
  // src/render/solarwind.js
  var WindLayer = class {
    #stream;
    #pos;
    #n = 0;
    #col = new Float64Array(3);
    #basis = new Float64Array(6);
    #mp;
    #mpN = 0;
    constructor(count = 260) {
    ...
    get standoffRe() {
      return this.#stream.standoffRe;
    }
    get activeCount() {
      return this.#n;
    }
    update(dtSec, weather2, sunDir, stretch = 0) {
      if (!this.visible) {
        this.#n = 0;
        return;
      }
      this.#n = this.#stream.step(dtSec, weather2, sunDir, this.#pos);
      bzColour(weather2.bzNt ?? 0, this.#col);
      this.alpha = flaringAlpha(
        weather2.bzNt ?? 0,
        weather2.windDensity ?? 5,
        weather2.windSpeedKmS ?? 400
      );
      basisFromNormal2(sunDir[0], sunDir[1], sunDir[2], this.#basis);
      this.#mpN = magnetopauseWireframe(
        this.standoffRe,
        this.alpha,
        sunDir,
        this.#basis,
    ...
    draw(p, kmToPx, sunDir) {
      ...
      for (let i = 0; i < this.#n; i++)
        p.vertex(
          this.#pos[i * 3] * kmToPx,
          this.#pos[i * 3 + 1] * kmToPx,
          this.#pos[i * 3 + 2] * kmToPx
        );
      p.endShape();
      p.noFill();
      p.stroke(110, 175, 245, 78);
      p.strokeWeight(1.1);
      p.beginShape(p.LINES);
  ...
  function fieldLineRadiusRe(L, lambdaRad) {
    return L * Math.cos(lambdaRad) ** 2;
  }
  function footLatitudeDeg(L) {
    return L <= 1 ? 0 : Math.acos(Math.sqrt(1 / L)) / D2R9;
  }
  function fieldLinePoint(axis, basis, L, lambdaRad, thetaRad, out, o = 0) {
  ...
  function trappedHelix(axis, basis, L, thetaRad, {
    samples = 220,
    turns = 26,
    gyroRe = 0.12,
    phase = 0,
    ...
    const latMax = footLatitudeDeg(L) * D2R9 * span;
  ...
  function torusSegmentCount(rings, sides) {
    return rings * sides * 2;
  }
  function torusWireframe(axis, basis, R, r, rings, sides, out) {
    ...
    for (let i = 0; i < rings; i++) {
      const t1 = i / rings * TAU4, t2 = (i + 1) / rings * TAU4;
      for (let j = 0; j < sides; j++) {
        const p1 = j / sides * TAU4, p2 = (j + 1) / sides * TAU4;
        seg(t1, p1, t1, p2);
        seg(t1, p1, t2, p1);
      }
    }
  ...
  function shellSegmentCount(meridians, steps) {
    return meridians * steps + (steps + 1) * meridians;
  }
  function fieldShellWireframe(axis, basis, L, meridians, steps, out) {
    const latMax = footLatitudeDeg(L) * D2R9;
    const a = new Float64Array(3), b = new Float64Array(3);
    let n = 0;
    const TAU4 = Math.PI * 2;
    const put = () => {
    ...
    for (let k = 0; k <= steps; k++) {
      ...
      for (let m = 0; m < meridians; m++) {
        fieldLinePoint(axis, basis, L, lam, m / meridians * TAU4, a);
        fieldLinePoint(axis, basis, L, lam, (m + 1) / meridians * TAU4, b);
        put();
      }
    }
    return n;
  }
  function helixWireframe(axis, basis, L, theta, opts, scratch2, out) {
    const n = trappedHelix(axis, basis, L, theta, opts, scratch2);
    for (let i = 0; i < n - 1; i++) {
      out.set(scratch2.subarray(i * 3, i * 3 + 3), i * 6);
      out.set(scratch2.subarray((i + 1) * 3, (i + 1) * 3 + 3), i * 6 + 3);
    }
    return n - 1;
  }
  function magneticBasis(axis, out) {
    return basisFromNormal2(axis[0], axis[1], axis[2], out);
  }

  // src/render/solids.js
  var AXIS = magneticAxis(new Float64Array(3));
  var BASIS = magneticBasis(AXIS, new Float64Array(6));
  ...
  var RadiationBelt = class _RadiationBelt extends Structure {
    #buf;
    #n;
    constructor(spec, rings = 44, sides = 14) {
      super(0, 0, 0, Infinity, new GeomStyle(spec.rgb, 62, 1));
      this.spec = spec;
      this.#buf = new Float64Array(torusSegmentCount(rings, sides) * 6);
      this.#n = torusWireframe(
        AXIS,
        BASIS,
        spec.L * R_EARTH_KM7,
        spec.halfWidth * R_EARTH_KM7,
        rings,
        sides,
        this.#buf
      );
    ...
    static inner() {
      return new _RadiationBelt(BELTS[0]);
    }
    static outer() {
      return new _RadiationBelt(BELTS[1]);
    }
    get segmentCount() {
      return this.#n;
    }
  ...
  var TrappedParticle = class extends Structure {
    #buf;
    #scratch;
    #n = 0;
  ...
  var FieldShell = class extends Structure {
    #buf;
    #n;
  ...
  var Magnetosphere = class {
    ...
    draw(p, kmToPx) {
      if (!this.visible) return;
      for (const q of this.#parts) q.draw(p, kmToPx);
    }
  };

  // src/sim/substorm.js
  var ONSET_THRESHOLD = 6e3;
  var DISSIPATION = 0.35;
  var EXPANSION_S = 15 * 60;
  var RECOVERY_S = 60 * 60;
  function couplingEy(speedKmS, bzNt) {
    return Math.max(0, -bzNt) * Math.max(0, speedKmS) / 1e3;
  }
  var SubstormCycle = class {
    #energy = 0;
    #phase = "quiet";
    #tPhase = 0;
    #onsets = 0;
    ...
    get loadFraction() {
      return Math.min(1, this.#energy / ONSET_THRESHOLD);
    }
    ...
    get auroraGain() {
      switch (this.#phase) {
        case "quiet":
        case "growth":
          return 1 - 0.25 * this.loadFraction;
        case "expansion": {
          const t = this.phaseProgress, ATTACK = 0.12;
          return t < ATTACK ? 0.75 + 2.85 * (t / ATTACK) : 1.7 + 1.9 * ((1 - t) / (1 - ATTACK)) ** 0.6;
        }
        case "recovery":
          return 1 + 0.7 * (1 - this.phaseProgress);
        default:
          return 1;
      }
    }
  ...
  // src/sim/demo.js
  var DEMO_LABEL = "SIMULATED — demonstration, not live data";
  var SCENARIO = [
    ...
    { t: 55, speedKmS: 780, density: 22, bzNt: -22, flux: 16e-5 },
    // X1.6
  ...
  var SCENARIO_MINUTES = SCENARIO[SCENARIO.length - 1].t;
  function demoConditions(minutes) {
    ...
    return {
      ...
      density: mix(a.density, b.density),
      bzNt: mix(a.bzNt, b.bzNt),
      flux: Math.exp(mix(Math.log(a.flux), Math.log(b.flux))),
      // log-interp
      minutes: m
    };
  }
  function demoKp(bzNt, speedKmS) {
    const drive = Math.max(0, -bzNt) * speedKmS / 1e3;
  ...
  var MOON_MEAN_DIST_KM = 384400;
  var MOON_RADIUS_KM = 1737.4;
  var MOON_DRAW_RE = 13;
  function moonDrawDistanceKm(trueDistKm, earthRadiusKm = 6371) {
    return MOON_DRAW_RE * earthRadiusKm * (trueDistKm / MOON_MEAN_DIST_KM);
  }
  ...
  var conjWatch = new ConjunctionWatch();
  var armillary = null;
  var armSig = "";
  ...
  var wind = new WindLayer(260);
  var magneto = new Magnetosphere();
  var FAR_FIT_RE = 24;
  var windSeries = null;
  var windSampleT = 0;
  var xraySeries = null;
  ...
  var substorm = new SubstormCycle();
  var flareWatch = new FlareWatch();
  ...
  async function loadWeather() {
    ...
    try {
      windSeries = await spaceweather_default.loadSeries();
    } catch {
      windSeries = null;
    }
    try {
      xraySeries = await spaceweather_default.loadXray(readXraySeries);
    } catch {
      xraySeries = null;
    }
    refreshHud();
  ...
  function refreshHud() {
    ...
    if (dbanner && demo.active) dbanner.textContent = DEMO_LABEL + " · storm scenario, " + Math.round(demo.minutes % SCENARIO_MINUTES) + "/" + SCENARIO_MINUTES + " min";
    setText("s-kp", demo.active ? "SIMULATED — demonstration conditions" : "Kp " + weather.kp.toFixed(1) + " · G" + weather.scaleG + " · " + weather.liveFields + "/" + weather.totalFields + " live (" + weather.source + ")");
    ...
    setText("s-wind", wind.visible ? "magnetopause " + wind.standoffRe.toFixed(1) + " Rₑ · flaring " + wind.alpha.toFixed(2) + " · " + wind.activeCount + " parcels (flow scaled)" : "hidden");
    setText("s-storm", substorm.phase + " · load " + Math.round(substorm.loadFraction * 100) + "% · gain " + substorm.auroraGain.toFixed(2) + " · " + substorm.onsets + " onsets");
    setText("s-xray", currentClass ? currentClass.label + (lastFlare ? " · last flare " + lastFlare.label : "") + " · " + flareWatch.count + " this session" : xraySeries ? "waiting" : "no X-ray series");
    setText("s-swseries", windSeries ? (spaceweather_default.seriesSource || "series") + " · " + windSeries.length + " samples" + (windSampleT ? " · replaying " + new Date(windSampleT).toISOString().slice(11, 16) + "Z" : "") : "snapshot only — " + (spaceweather_default.seriesError || "not loaded"));
    setText("s-frame", wind.visible || magneto.visible ? "wide — fits " + FAR_FIT_RE + " Rₑ (magnetosphere)" : "close — orbital shells");
    setText("s-belt", magneto.visible ? magneto.parts + " objects · " + magneto.segmentCount + " segments" : "hidden");
    ...
    setText("s-conj", state.conjCount + " encounters · " + conjWatch.activeCount + " close now");
  }
  new p5((p) => {
    let lastReal = 0;
    p.setup = () => {
    ...
    p.draw = () => {
      ...
      viewTarget = farOn ? 0.44 * rOuterKm / (0.46 * FAR_FIT_RE * R_EARTH_KM4) : 1;
      viewScale += (viewTarget - viewScale) * 0.07;
      ...
      drawEarth(p, R_EARTH_KM4 * kmToPx, sunEF);
      aurora.update(
        auroraKp,
        sun.subsolarLonDeg,
      ...
      p.blendMode(p.ADD);
      aurora.draw(p, R_EARTH_KM4 * kmToPx, kmToPx);
      magneto.update(p.deltaTime / 1e3);
      magneto.draw(p, kmToPx);
      p.blendMode(p.BLEND);
      if (focused >= 0 && track.n > 1) {
        const rT = R_EARTH_KM4 * kmToPx * 1.006;
        p.strokeWeight(1.6);
      ...
      const sunD = SUN_DISTANCE_RE * R_EARTH_KM4 * kmToPx;
      ...
      const dSim = Math.max(0, Math.min(3600, (state.simT - lastSimT) / 1e3));
      lastSimT = state.simT;
      const demoC = state.paused ? null : demo.update(dSim);
      const live = sampleSeries(windSeries, state.simT);
      const conditions = demo.active && demoC ? { ...weather, ...demoC } : live ? { ...weather, ...live } : weather;
      ...
      if (!state.paused && substorm.update(dSim, conditions) === "onset")
        sonifier.crossing(220, true);
      const xr = demo.active && demoC ? { flux: demoC.flux } : sampleSeries(xraySeries && xraySeries.map(
        (r) => ({ tMs: r.tMs, flux: r.flux })
      ), state.simT);
      if (xr) {
        ...
        if (ev) {
          lastFlare = ev;
          flareGlow = 1;
          sonifier.ping(1400, 1900);
        }
      }
      flareGlow = Math.max(0, flareGlow - p.deltaTime / 2600);
      wind.update(p.deltaTime / 1e3, conditions, sun.eciDir, substorm.tailStretch);
    ...
    p.keyPressed = () => {
      ...
      if (k === "g") {
        ...
        magneto.visible = !magneto.visible;
        refreshHud();
      } else if (k === "d") {
        demo.toggle();
        refreshHud();
